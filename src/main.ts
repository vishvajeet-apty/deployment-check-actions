...
async function run(): Promise<void> {
    try {
        ...
        const isTargetFileExists = await isFileExists({Bucket: bucketName, Key: `abc/${branchName}.json`})
        if (! isTargetFileExists) { // now check the difference if any
            ...
            targetBranchData = await getS3Object({Bucket: bucketName, Key: `abc/${branchName}.json`})
        }
